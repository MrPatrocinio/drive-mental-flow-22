-- FASE 2: Correção de funções com search path mutável e proteção de dados de business
...
-- Remove política insegura que permite visualização total
DROP POLICY IF EXISTS "Everyone can view landing content" ON public.landing_content;

-- Permite visualização apenas para conteúdo público e usuários autenticados
CREATE POLICY "Public can view landing content" 
ON public.landing_content 
FOR SELECT 
USING (true); -- Landing content precisa ser público para funcionamento do site

-- Apenas admins podem modificar conteúdo de landing
...
-- Proteger dados de business: Restringir acesso à tabela background_music
-- Remove política insegura se existir
DROP POLICY IF EXISTS "Everyone can view background music" ON public.background_music;

-- Permite visualização apenas para usuários autenticados
CREATE POLICY "Authenticated users can view background music" 
ON public.background_music 
FOR SELECT 
USING (auth.uid() IS NOT NULL);

-- Apenas admins podem modificar música de fundo
